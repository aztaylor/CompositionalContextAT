...
@mtkmodel rates begin
  ...
  @variables begin
    σtₛ(t)=-6, [description="supercoil state of mSpinach ORF", unit="bp*turn^-1"]
    σtₘ(t)=-3, [description="supercoil state of MG ORF"]
    σpₗ(t)=-6, [description="supercoil state of mSpinach promoter", unit="bp*turn^-1"]
...
@mtkmodel dnaComplexDynamics begin
  @extend rates()
  @parameters begin
    rnapᵗᵒᵗ=18.931e3, [description="Total RNAP, in myTXTL no degredataion", unit=u"nM", connect = Flow]
...
@mtkmodel σMaintenanceDynamics begin
  ...
  @equations begin
    ...
    mpₜ~topo₀*τ*(σpₜ₋)/kσₘₘ/(σ₀+(σpₜ-σ₀)^2)/kσₘₘ+gyr₀*γ*(σpₜ₊)/kσₘₘ/(σ₀+(σpₜ-σ₀)^2)/kσₘₘ
    mtₘ~topo₀*τ*(σtₘ₋)/kσₘₘ/(σ₀+(σtₘ-σ₀)^2)\kσₘₘ+gyr₀*γ*(σtₘ₊)/kσₘₘ/(σ₀+(σtₘ-σ₀)^2)/kσₘₘ
  end
end
