...
String macro to easily recall biological customary units located in the bioUnits
package. Although all unit symbols in that package are suffixed with `_bio`,
...
function replace_value(sym::Symbol)
  s = Symbol(sym, :_ub)
  if !(isdefined(UnitfulBio, s) && ustrcheck_bool(getfield(UnitfulBio, s)))
      error("Symbol $s could not be found in UnitfulBio.")
  end

  return getfield(UnitfulBio, s)
end
